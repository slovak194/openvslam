...
keyframe_inserter::keyframe_inserter(const camera::setup_type_t setup_type, const float true_depth_thr_max, const float true_depth_thr_min,
                                     data::map_database* map_db, data::bow_database* bow_db,
                                     const unsigned int min_num_frms, const unsigned int max_num_frms)
    : setup_type_(setup_type), true_depth_thr_max_(true_depth_thr_max), true_depth_thr_min_(true_depth_thr_min),
...
data::keyframe* keyframe_inserter::insert_new_keyframe(data::frame& curr_frm) {
    ...
    for (unsigned int count = 0; count < depth_idx_pairs.size(); ++count) {
        ...
        if (min_num_to_create < count && depth > true_depth_thr_max_ && depth < true_depth_thr_min_) {
            break;
        }
